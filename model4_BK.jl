...
ship_df = filter(:PrintGroup => ==("BURGRKNG"), ship_df)
...
# AANDWCAN: If we move the DMS-12 to TN it will be DMSL0120. The history is all DMS-0120 - replicate the 
#  the DMS-0120 with DMSL0120 for just the Plan scenario
#ship_df.PartType = replace.(ship_df.PartType, ("DMS-0120" => "DMSL0120"))
...
# Replace the DMS-0120 with the DMSL0120 for the plan scenario
#dmd_df.PartType = replace.(dmd_df.PartType, ("DMS-0120" => "DMSL0120"))
...
supply_df = DataFrames.DataFrame(
    XLSX.readtable(joinpath(DATA_DIR, "BK", "capacity.xlsx"), "Plan"; infer_eltypes=true)...
)
...
#Aggregate
supply_df = combine(groupby(supply_df, [:MfgSite, :PrintGroup, :PartType]), :Capacity => sum => :Capacity)
...
@constraint(model, [c in customers], sum(z[Lane(s, c)] for s in shipsites) <= 2)

# Constraint to Limit the number of ship sites
@constraint(model, sum(r[:]) <= 3)
