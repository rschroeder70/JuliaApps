...
supply_df = combine(groupby(ship_df, [:ShipSiteName, :PTypeFix]), :MUnits => sum => :MUnits)
...
supply_dict = Dict(Supply.(supply_df.ShipSiteName, supply_df.PTypeFix) .=> supply_df.MUnits)

# Just include where we have distances
dmd_df = innerjoin(DataFrames.select(demand_df, :MSTName, :Loc, :FTerms, :PTypeFix, :MUnits), 
...
dmd_key = Demand.(Cust.(dmd_df.MSTName, dmd_df.Loc, dmd_df.FTerms), dmd_df.PTypeFix)

demand_dict = Dict(dmd_key .=> dmd_df.MUnits)
keys(demand_dict)
demand_dict[Demand(Cust("ACME PAPER & SUPPLY, INC.", "JESSUP, MD", "PPD"), :LID)]

# Add missing elements with a 0 demand
1 in keys(demand_dict)
...
# Define variables
@variable(model3, x[lanes, ptypes] >= 0)
...
# Objective
@objective(model3, Min, sum(c[l] * x[l, t] for l in lanes, t in ptypes))

optimize!(model3)

#print(model2_output)

status = termination_status(model3)
...
Test.@test termination_status(model3) == MOI.OPTIMAL
Test.@test primal_status(model3) == MOI.FEASIBLE_POINT
println("the objective is: ")
println(objective_value(model3))
@printf "%d" objective_value(model3)
@pipe [objective_value(model3)] |> PrettyTables.pretty_table(_; formatters = PrettyTables.ft_printf("%'d"))
