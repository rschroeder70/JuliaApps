# Model 2 + 2 Parts, Lid and Cup
# Modele- Add Lid and Cup Constraints by Plant
...
demand_df = combine(groupby(ship_df, [:MSTName, :Loc, :FrtTermsFix, :PTypeFix]), :MUnits => sum => :MUnits)
...
dmd_df = innerjoin(DataFrames.select(demand_df, :MSTName, :Loc, :FTerms, :PTypeFix, :MUnits), DataFrames.select(distw_df, :Dest), on = [:Loc => :Dest])
...
dmd_df.PTypeFix = Symbol.(dmd_df.PTypeFix)
...
demand_dict = Dict{Demand, Float64}()
# Trying this with nested tuples
nt1 = Tables.rowtable(DataFrames.select(dmd_df, :MSTName, :Loc, :FTerms))
st1 = structfromnt.(Cust, Tables.rowtable(DataFrames.select(dmd_df, :MSTName, :Loc, :FTerms)))
st1[1].MSTName
nt2 = Tables.rowtable(DataFrames.select(dmd_df, :PTypeFix))
nt3 = (nt1, Tables.rowtable(DataFrames.select(dmd_df, :PTypeFix)))
...
dmd_st = Demand[]
for i in 1 : nrow(dmd_df)
    #println(cust_st[i])
    push!(dmd_st, Demand(cust_st[i], dmd_df.PTypeFix[i]))
end
dmd_st[1]
dmd_st[1].Customer.MSTName
# Looks like these next two do the same thing
merge.(Tables.rowtable(DataFrames.select(dmd_df, :MSTName, :Loc, :FTerms)), Tables.rowtable(DataFrames.select(dmd_df, :PTypeFix)))
Tables.rowtable(DataFrames.select(dmd_df, :MSTName, :Loc, :FTerms, :PTypeFix))

dmd_df[[1],:]
dmd_df.PTypeFix[1]

Tables.rowtable(dmd_df[[1], :])

demand_dict = Dict(structfromnt.(Demand, Tables.rowtable(Cust, Tables.rowtable(DataFrames.select(dmd_df, :MSTName, :Loc, :FTerms), :PTypeFix))) .=> dmd_df.MUnits)

pairs(Tables.rowtable(DataFrames.select(dmd_df, :MSTName, :Loc, :FTerms)))
