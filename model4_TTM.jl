...
#   customer.
#   Product is made at the Mfg Site and then shipped to a Ship Site.  
...
#Aggregate
supply_df = combine(groupby(supply_df, [:MfgSite, :PartType]), :Capacity => sum => :Capacity)
...
for s in supplies
    println(s.plant, " ", s.ptype, " ", supply_dict[s])
end
...
demand_dict = Dict(Demand.(Cust.(demand_df.MSTName, demand_df.Loc, demand_df.FTerms), demand_df.PartType) .=> demand_df.MUnits)
...
#@variable(model, y1[bf_lanes, ptypes] >= 0)
@variable(model, y[supplies, plants] >= 0)
...
@constraint(model,
    supply_con[s in supplies], sum(y[Supply(s.plant, s.ptype), d] for d in shipsites) <= supply_dict[Supply(s.plant, s.ptype)])

# Customer Demand constraint
@constraint(model,
    demand_con[d in demands], sum(x[Lane(s, d.customer), d.ptype] for s in shipsites) == demand_dict[Demand(d.customer, d.ptype)])

# shipsite inbound = outbound for net of 0
@constraint(model, inout_con[s in shipsites, t in ptypes], sum(y[Supply(p.plant, p.ptype), s] for p in supplies if p.ptype == t) == 
    sum(x[Lane(s, c), t] for c in customers))

#limit the ship sites to 1 per customer
@constraint(model, maxship_con[c in customers], sum(z[Lane(s, c)] for s in shipsites) == 1)
...
@constraint(model, avail_con[s in shipsites, d in demands], 
    x[Lane(s, d.customer), d.ptype] <= min(demand_dict[Demand(d.customer, d.ptype)]) * z[Lane(s, d.customer)])
...
# Objective
@objective(model, Min, sum(bf_dict[BFLane(s.plant, d)] * y[Supply(s.plant, s.ptype), d] for s in supplies, d in shipsites) + 
    sum(cost[l] * x[l, t] for l in lanes, t in ptypes))

#print(model)
#write_to_file(model, "model4_TTM.lp")
...
for s in shipsites
    for c in customers
        for p in custprod[c]
            ...
            if !isapprox(value(x[Lane(s, c), p]), 0.0, atol = 1e-10, rtol = 0) #&& l.Cust.FTerms == "PPD"
                frt = if c.fterms == "PPD" value(x[Lane(s, c), p]) * cost[Lane(s, c)] else 0 end
                tupl = (s, c.mstname, c.loc, c.fterms, p, value(x[Lane(s, c), p]), frt)
...
XLSX.writetable(joinpath(OUT_DIR, "model4TTMob_output.xlsx"), overwrite = true, 
