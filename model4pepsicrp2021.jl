...
demandpep_df = combine(groupby(shippep_df, [:MSTName, :Loc, :FrtTermsFix, :PartType]), :MUnits => sum => :MUnits)
rename!(demandpep_df, :FrtTermsFix => :FTerms)

# Make sure we haven't lost anything
@pipe combine(demandpep_df, :MUnits => sum => :MUnits) |> PrettyTables.pretty_table(_; formatters = PrettyTables.ft_printf("%'d"))


# Build the various data structures to hold the supply and demand info

supplypep_df = combine(groupby(shippep_df, [:MfgSiteName, :PartType]), :MUnits => sum => :MUnits)
@pipe supplypep_df |> PrettyTables.pretty_table(_; formatters = PrettyTables.ft_printf("%'d"))

#Copy this to Excel and fix it up
clipboard(sprint(show, "text/tab-separated-values", supplypep_df))

supplypep_df = DataFrames.DataFrame(
    XLSX.readtable(joinpath(DATA_DIR, "Pepsicrp", "capacity.xlsx"), "Sheet1"; infer_eltypes=true)...
)
dropmissing!(supplypep_df)
rename!(supplypep_df, :MUnits => :Capacity, :MfgSiteName => :MfgSite)
supplypep_df[!, :Capacity] = convert.(Float64, supplypep_df[!, :Capacity])
#select!(supplypep_df, Not([:Shipped]))
subset!(supplypep_df, :Capacity => ByRow(!=(0)))
#supplypep_df = supplypep_df[((supplypep_df.PartType .== "SDRA0160") .| (supplypep_df.PartType .== "DMR-0320")), :]

# Copy to the clipboard
clipboard(sprint(show, "text/tab-separated-values", demandpep_df))
...
ptypes = unique(supplypep_df.PartType)

#[
#:LID; :CUP
#]

supply_dict = Dict(Supply.(supplypep_df.MfgSite, supplypep_df.PartType) .=> supplypep_df.Capacity)
...
cust_nt = Tables.rowtable(DataFrames.select(demandpep_df, :MSTName, :Loc, :FTerms))
unique(cust_nt)
...
customers = Cust.(demandpep_df.MSTName, demandpep_df.Loc, demandpep_df.FTerms)
...
demand_key = Demand.(Cust.(demandpep_df.MSTName, demandpep_df.Loc, demandpep_df.FTerms), demandpep_df.PartType)

demand_dict = Dict(demand_key .=> demandpep_df.MUnits)
keys(demand_dict)
...
@pipe combine(demandpep_df, :MUnits => sum => :MUnits) |> 
    PrettyTables.pretty_table(_; formatters = PrettyTables.ft_printf("%'d"))
...
XLSX.writetable(joinpath(OUT_DIR, "model1ob_output.xlsx"), overwrite = true, 
...
XLSX.writetable(joinpath(OUT_DIR, "Data", "Pepsrcrp", "model1bf_output.xlsx"), overwrite = true, 
