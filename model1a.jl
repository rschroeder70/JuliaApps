# Model1a - No capacity constraints.  Single Part.
...
dmd_df = innerjoin(DataFrames.select(demand_df, :MSTName, :Loc, :FTerms, :MUnits), 
    DataFrames.select(distw_df, :Dest), on = [:Loc => :Dest])
...
# Set of Origins
plants = unique(ship_df.ShipSiteName)
...
# Objective
@objective(model1a, Min, sum(c[l] * x[l] for l in lanes))

# @objective model1a Min begin
#     sum(c[l] * x[l] for l in lanes)
# end

optimize!(model1a)

print(model1a_output)

status = termination_status(model1a)
if (status == MOI.OPTIMAL) || (status == MOI.LOCALLY_SOLVED)
    println("Objective value: ",objective_value(model1a))
    println(value.(x))
...
Test.@test termination_status(model1a) == MOI.OPTIMAL
Test.@test primal_status(model1a) == MOI.FEASIBLE_POINT
println("the objective is: ")
println(objective_value(model1a))
@printf "%d" objective_value(model1a)
@pipe [objective_value(model1a)] |> PrettyTables.pretty_table(_; formatters = PrettyTables.ft_printf("%'d"))
...
@pipe combine(out_df, :MUnits => sum => :MUnits) |> 
...
#XLSX.writetable("model1a_output.xlsx", overwrite = true, collect(DataFrames.eachcol(out_df)), DataFrames.names(out_df))

using Statistics
geocodes_fix = combine(groupby(geocodes_df, [:Dest]), :lat => mean => :lat, :lon => mean => :lon)
model1a_dmd = leftjoin(combine(groupby(out_df, [:Dest, :FTerms]), :MUnits => sum => :MUnits), 
    geocodes_fix, on = :Dest => :Dest)
model1a_dmd = filter(:FTerms => !=("CPU"), model1a_dmd)
